...
collision_circle2:
  ...
  ;FLD ballX
  ;FLD ballVectorX
  ;FSUBP ST(1), ST(0) ; ST(0) = начало вектора (по х)                   х1
  ;FSTP x1
  
  ;FLD ballY
  ;FLD ballVectorY
  ;FSUBP ST(1), ST(0) ; ST(0) = начало вектора (по y)                   у1
  ;FSTP y1
  ...
  ;FLD ballVectorY
  ;FMUL ballVectorY
  ;FADDP ST(1), ST(0) ; сумма в st(0)
  ;FSQRT ; корень в st(0)
  ;FSTP lenBC                                ; длина BC

  ;найти cos alpha
  ;FLD ballVectorX
  ;FMUL min1 ; st(0) = vectX * (-1)
  ;FLD ballX ; ST(0) = ballX, ST(1) = vectX * (-1)
  ;FMULP ST(1), ST(0) ; st(0)

  ;FLD ballVectorY ;st(0), st(1)
  ;FMUL min1 ; st(0) = vectY * (-1)
  ;FLD ballY ; ST(0) = ballY, ST(1) = vectY * (-1)
  ;FMULP ST(1), ST(0) ; st(0), st(1)               

  ;FADDP ST(1), ST(0) ;                   st(0) = числитель;

  ;FLD ballVectorX ; st(0)= ballVectorX, st(1) = числитель
  ;FMUL ballVectorX ; в квадр
  ;FLD ballVectorY ;  st(0)= ballVectorY, st(1)= ballVectorX, st(2) = числитель
  ;FMUL ballVectorY ; в квадр 
  ;FADDP ST(1), ST(0) ; st(0), st(1)
  ;FSQRT ; корень в st(0)
  ...
  ;FDIVP ST(1),ST(0) ;                                     st(0) =  cos  alpha

  ;найти AB (длинв нормали)
  ...
  ; ищем точку x2
  ;FLD lenAC
  ;FMUL lenAC ; st(0) = AC^2
  ;FLD x1
  ;FMUL x1 ; st(0)=x1^2, st(1) = AC^2
  ;FMUL min1
  ;FADDP ST(1), ST(0)
  ;FLD y1
  ;FMUL y1
